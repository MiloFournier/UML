    ...
    bool Service::verifierEtatCapteur(Capteur capteurParam) {
        ...
        // Vérification de la distance entre capteurParam et capteurTest
            if (calculDistance(capteurParam, capteurTest) < distanceDeVerification) {
                ...
                for(const auto &i : capteurParam.getLMesure_O3()) {
                    for(const auto &j : capteurTest.getLMesures_O3()) {
                        if(i->first == j->first) {
                ...
                for(const auto &i : capteurParam.getLMesure_NO2()) {
                    for(const auto &j : capteurTest.getLMesures_NO2()) {
                        if(i->first == j->first) {
                            if(abs(i->second.getValeur() - j->second.getValeur()) > limiteDeMesure) {
                                compteurErreurs++;
                            }
                ...
                for(const auto &i : capteurParam.getLMesure_SO2()) {
                    for(const auto &j : capteurTest.getLMesures_SO2()) {
                        if(i->first == j->first) {
                            if(abs(i->second.getValeur() - j->second.getValeur()) > limiteDeMesure) {
                                compteurErreurs++;
                            }
                ...
                for(const auto &i : capteurParam.getLMesure_PM10()) {
                    for(const auto &j : capteurTest.getLMesures_PM10()) {
                        if(i->first == j->first) {
                            if(abs(i->second.getValeur() - j->second.getValeur()) > limiteDeMesure) {
                                compteurErreurs++;
                            }
